...
- **Caching** — fonts and CSS/JS immutable for a year, images 30 days. CSS and JS URLs carry a content hash
  (`main.css?v=0ee6fc9b`) generated by `build.py`, so editing a file changes every URL referencing it. That is
  what makes the immutable policy safe: fresh HTML can never be served against a stale stylesheet.
...
Articles live at `tools/pages/insight/<slug>.html` and publish to `/insight/<slug>`. `build.py` handles the
nested slug, the breadcrumb (Home → Insight → article) and the share card.

Every quantitative claim carries a numbered citation linking to a source in the article's own source list. If
you edit an article, keep that property — it is the reason the section is worth having. Where evidence is
genuinely unresolved (for example, Saudi Arabia has no confirmed ISSB adoption date), the articles say so
rather than implying certainty; do not "tidy" those hedges away.
